...
{     
  ...
  //Do not send these or they will be overwritten on power restore
  //Wire.send(0x01);  //set seconds
  //Wire.send(0x01);  //set minutes
  //Wire.send(0x01);  //set hour

  Wire.beginTransmission(Rtcc_Addr);    // Issue I2C start signal
  Wire.send(RTCC_DAY_ADDR);    // start address
...
{  
    ...
    if (s == 255 && m == 255 && h == 255) return; // values if not connected, do not update

    //0x7f = 0b01111111
    sec = bcdToDec(s & 0x7f);
    minute = bcdToDec(m & 0x7f);
    //0x3f = 0b00111111
    hour = bcdToDec(h & 0x3f);
}
...
long Rtc_Pcf8563::getClockSeconds(){
  getTime(); // to get most recent hour, minute, and sec
  long hour_s = long(hour)*3600;
  long minute_s = long(minute)*60;
  long second_s = long(sec);
